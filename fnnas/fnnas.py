"""
脚本名: 飞牛 Nas
环境变量: FNNASCLUB={"cookie": ""}
cookie 获取， 通过抓取飞牛论坛签到包的 cookies 填写
功能实现: 签到奖励
来源: https://github.com/Sitoi/dailycheckin
修改者: zhilv
"""
...
class FnNasClub:
    ...
    def get_sign_param_from_page(session):
        ...
        response = session.get(
            "https://club.fnnas.com/plugin.php?id=zqlj_sign", timeout=15
        )
        ...
        try:
            ...
            print(
                f"status_code: {response.status_code}, text: {response.text[:2000]}, exception: {e}"
            )
        return None
    ...
    def sign(session, sign_param):
        if not sign_param:
            ...
            return msg
        response = session.get(
            f"https://club.fnnas.com/plugin.php?id=zqlj_sign&sign={sign_param}",
            timeout=15,
        )
        try:
            html = response.text
...
if __name__ == "__main__":
    _check_item = json.loads(os.environ.get("FNNASCLUB", "{}"))
